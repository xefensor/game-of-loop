...
func get_neighbours(generation: Generation, position: Vector2i) -> Array:
    ...
    var cols = generation.cells.size()       # počet sloupců (x)
    var rows = generation.cells[0].size()    # počet řádků (y)

    for dy in range(-1, 2):  # -1, 0, 1
        for dx in range(-1, 2):
            if dx == 0 and dy == 0:
                continue  # přeskočíme aktuální buňku
            var nx = position.x + dx
            var ny = position.y + dy
            if nx >= 0 and nx < cols and ny >= 0 and ny < rows:
                neighbours.append(generation.cells[nx][ny])  # x první, y druhé
